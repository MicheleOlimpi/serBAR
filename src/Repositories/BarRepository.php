...
class BarRepository
{
    ...
    {
        if (!empty($d['id'])) {
            $this->pdo->prepare('UPDATE calendar_days SET day_date=?, recurrence_name=?, santo=?, is_special=?, day_type_id=? WHERE id=?')
                ->execute([$d['day_date'], $d['recurrence_name'], $d['santo'], $d['is_special'], $d['day_type_id'] ?: null, (int) $d['id']]);
            return;
        }

        $this->pdo->prepare('INSERT INTO calendar_days (day_date, recurrence_name, santo, is_special, day_type_id) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE recurrence_name=VALUES(recurrence_name), santo=VALUES(santo), is_special=VALUES(is_special), day_type_id=VALUES(day_type_id)')
            ->execute([$d['day_date'], $d['recurrence_name'], $d['santo'], $d['is_special'], $d['day_type_id'] ?: null]);
    }
    ...
    public function boardDays(int $boardId): array
    {
        $stmt = $this->pdo->prepare(
            ...
            LEFT JOIN (
                SELECT DATE_FORMAT(cd.day_date, '%m-%d') AS month_day, cd.santo
                FROM calendar_days cd
                INNER JOIN (
                    SELECT DATE_FORMAT(day_date, '%m-%d') AS month_day, MAX(day_date) AS latest_day_date
                    FROM calendar_days
                    GROUP BY DATE_FORMAT(day_date, '%m-%d')
                ) latest ON latest.month_day = DATE_FORMAT(cd.day_date, '%m-%d')
                    AND latest.latest_day_date = cd.day_date
            ) c ON c.month_day = DATE_FORMAT(bd.day_date, '%m-%d')
            WHERE bd.board_id = ?
            ORDER BY day_date"
        );
