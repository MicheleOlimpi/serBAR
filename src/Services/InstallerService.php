...
class InstallerService
{
    ...
    {
        return [
            "CREATE TABLE IF NOT EXISTS users (id INT AUTO_INCREMENT PRIMARY KEY, username VARCHAR(50) UNIQUE, alias VARCHAR(100) NOT NULL DEFAULT '', last_name VARCHAR(100), first_name VARCHAR(100), email VARCHAR(255) NOT NULL DEFAULT '', receive_system_mail ENUM('si','no') NOT NULL DEFAULT 'no', password_hash VARCHAR(255), role VARCHAR(20) NOT NULL DEFAULT 'user', phone VARCHAR(30) NOT NULL DEFAULT '', status ENUM('attivo','inattivo') NOT NULL DEFAULT 'attivo', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
            ...
            "CREATE TABLE IF NOT EXISTS app_settings (setting_key VARCHAR(100) PRIMARY KEY, setting_value VARCHAR(255) NOT NULL, setting_meaning VARCHAR(50) NOT NULL DEFAULT '')",
    ...
    {
        $stmt = $pdo->query("SHOW COLUMNS FROM users LIKE 'alias'");
        if ($stmt === false || !$stmt->fetch()) {
            $pdo->exec("ALTER TABLE users ADD COLUMN alias VARCHAR(100) NOT NULL DEFAULT '' AFTER username");
        }

        $stmt = $pdo->query("SHOW COLUMNS FROM users LIKE 'email'");
        if ($stmt === false || !$stmt->fetch()) {
            $pdo->exec("ALTER TABLE users ADD COLUMN email VARCHAR(255) NOT NULL DEFAULT '' AFTER first_name");
        }

        $stmt = $pdo->query("SHOW COLUMNS FROM users LIKE 'receive_system_mail'");
        if ($stmt === false || !$stmt->fetch()) {
            $pdo->exec("ALTER TABLE users ADD COLUMN receive_system_mail ENUM('si','no') NOT NULL DEFAULT 'no' AFTER email");
        } else {
            $pdo->exec("UPDATE users SET receive_system_mail = 'no' WHERE receive_system_mail NOT IN ('si','no') OR receive_system_mail IS NULL");
            $pdo->exec("ALTER TABLE users MODIFY COLUMN receive_system_mail ENUM('si','no') NOT NULL DEFAULT 'no'");
        }
        $pdo->exec("UPDATE users SET receive_system_mail = 'no' WHERE receive_system_mail = 'si' AND TRIM(COALESCE(email, '')) = ''");
        ...
        if ($stmt === false || !$stmt->fetch()) {
            $pdo->exec("ALTER TABLE users ADD COLUMN status ENUM('attivo','inattivo') NOT NULL DEFAULT 'attivo' AFTER phone");
        } else {
            $pdo->exec("UPDATE users SET status = 'attivo' WHERE status NOT IN ('attivo','inattivo') OR status IS NULL");
            $pdo->exec("ALTER TABLE users MODIFY COLUMN status ENUM('attivo','inattivo') NOT NULL DEFAULT 'attivo'");
        }
    }
    ...
    {
        $pdo->exec("CREATE TABLE IF NOT EXISTS app_settings (setting_key VARCHAR(100) PRIMARY KEY, setting_value VARCHAR(255) NOT NULL, setting_meaning VARCHAR(50) NOT NULL DEFAULT '')");

        $stmt = $pdo->query("SHOW COLUMNS FROM app_settings LIKE 'setting_meaning'");
        if ($stmt === false || !$stmt->fetch()) {
            $pdo->exec("ALTER TABLE app_settings ADD COLUMN setting_meaning VARCHAR(50) NOT NULL DEFAULT '' AFTER setting_value");
        }
    }
